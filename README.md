...
Extra methods `F` / `U` / `C` / `P`: parent-first AIM with equal split, convex split with uniform FK, convex allocation without parent-first, and optional PrivLava. If PrivLava is not installed, the runner writes a skip JSON and does not invent a numeric row. See `extras/README.md`.
