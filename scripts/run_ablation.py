...
def main() -> None:
    ...
    ablation_methods = {
        ...
        "U_uniform_fk": run_uniform_fk,
        "C_convex_no_parent_first": run_convex_no_parent_first,
    }

    ablation_results: list[dict] = []

    if cfg.get("experiment", {}).get("include_aim_equal_f", False):
        ablation_methods["F_parent_first_aim_equal"] = run_parent_first_aim_equal
    ...
    for name, fn in ablation_methods.items():
        ...
        except OptionalExtraError as e:
            logger.warning("Skipping ablation %s: %s", name, e)
            continue
        elapsed = time.time() - t0
    ...
    write_results_with_seed_std(ablation_results, output_dir, stem="ablation_components")
