...
class QueryAccuracyEvaluator:
    ...
    def __init__(
        ...
        self.join_clip_mult = 10.0
        self.join_card_real: list[float] = []
        self.join_card_syn: list[float] = []
        self.join_card_syn_clipped: list[float] = []
    ...
    def evaluate(
        ...
        for qt, errors in results.items():
            if errors:
                ...
                mixed.extend(errors)
                if qt in ("1way", "2way", "3way"):
                    marginal.extend(errors)
            else:
                metrics[f"query_error_{qt}"] = float("nan")

        # Headline avg = single-table TV only. Do not mix with relative join error.
        metrics["query_error_marginal"] = (
            float(np.mean(marginal)) if marginal else float("nan")
        )
        metrics["query_error_avg"] = metrics["query_error_marginal"]
        metrics["query_error_mixed_legacy"] = (
            float(np.mean(mixed)) if mixed else float("nan")
        )
        if self.join_card_real:
            metrics["join_card_real_mean"] = float(np.mean(self.join_card_real))
            metrics["join_card_syn_mean"] = float(np.mean(self.join_card_syn))
            metrics["join_card_syn_clipped_mean"] = float(
                np.mean(self.join_card_syn_clipped)
            )
            metrics["join_card_unclipped_absdiff"] = float(
                np.mean(np.abs(np.asarray(self.join_card_real) - np.asarray(self.join_card_syn)))
            )
        return metrics
    ...
    def _cross_table_query(
        ...
        """Bounded FK-join fidelity in [0, 1]: clipped cardinality + join TVs.

        Does **not** use relative error of a joined mean (that was unbounded).
        """
        ...
        if fk.child_col not in real_child.columns or fk.parent_col not in real_parent.columns:
            return None
        if fk.child_col not in syn_child.columns or fk.parent_col not in syn_parent.columns:
            return None

        parent_keep = [fk.parent_col]
        parent_meta = self.schema.tables[parent]
        extra = [
            c for c in parent_meta.attributes
            if c != fk.parent_col and c in real_parent.columns and c in syn_parent.columns
        ]
        if extra:
            parent_keep.append(str(self.rng.choice(extra)))

        real_joined = real_child.merge(
            real_parent[parent_keep], left_on=fk.child_col, right_on=fk.parent_col, how="inner",
        )
        syn_joined = syn_child.merge(
            syn_parent[parent_keep], left_on=fk.child_col, right_on=fk.parent_col, how="inner",
        )

        parts: list[float] = []
        jr = float(len(real_joined))
        js = float(len(syn_joined))
        clip_cap = self.join_clip_mult * max(jr, 1.0)
        js_clipped = min(js, clip_cap)
        self.join_card_real.append(jr)
        self.join_card_syn.append(js)
        self.join_card_syn_clipped.append(js_clipped)
        if js > clip_cap and len(syn_joined) > 0:
            syn_joined = syn_joined.sample(
                n=int(clip_cap), random_state=int(self.rng.integers(0, 1_000_000))
            )
        parts.append(min(abs(jr - js_clipped) / max(jr, 1.0), 1.0))
        # Do not TV the FK column: parent-first overwrites it, so both
        # integrity-preserving methods look identical there.
        child_attrs = [
            c for c in real_child.columns
            if c != fk.child_col and c in real_joined.columns and c in syn_joined.columns
        ]
        if child_attrs:
            col = str(self.rng.choice(child_attrs))
            parts.append(_tv_series(real_joined[col], syn_joined[col]))
        if len(parent_keep) > 1:
            col = parent_keep[1]
            if col in real_joined.columns and col in syn_joined.columns:
                parts.append(_tv_series(real_joined[col], syn_joined[col]))
        return float(np.mean(parts))


def _tv_series(real_s: pd.Series, syn_s: pd.Series, num_bins: int = 20) -> float:
    """Total variation in [0, 1] between two columns."""
    real_num = pd.to_numeric(real_s, errors="coerce")
    syn_num = pd.to_numeric(syn_s, errors="coerce")
    numeric = real_num.notna().mean() > 0.8 and syn_num.notna().mean() > 0.8
    if numeric and real_num.nunique(dropna=True) > 12:
        r = real_num.dropna().to_numpy()
        s = syn_num.dropna().to_numpy()
        ...
        rh, _ = np.histogram(r, bins=edges, density=False)
        sh, _ = np.histogram(s, bins=edges, density=False)
        rh = rh / max(rh.sum(), 1)
        sh = sh / max(sh.sum(), 1)
        return float(0.5 * np.abs(rh - sh).sum())
    rc = real_s.astype(str).value_counts(normalize=True)
    sc = syn_s.astype(str).value_counts(normalize=True)
    keys = set(rc.index) | set(sc.index)
    return float(0.5 * sum(abs(rc.get(k, 0.0) - sc.get(k, 0.0)) for k in keys))
...
def ml_utility(
    ...
    trains LightGBM (paper default) on synthetic joined data, tests on real rows.
    """
    ...
    try:
        ...
        X_train_real, X_test_real, y_train_real, y_test_real = train_test_split(
            real_X, real_y, test_size=test_fraction, random_state=seed
        )

    clf_syn = _make_classifier(model, seed)
    clf_syn.fit(syn_X, syn_y)
    pred_syn = clf_syn.predict(X_test_real)

    clf_real = _make_classifier(model, seed)
...
def evaluate_all(
    ...
    qeval.join_clip_mult = float(join_clip_mult)
    ...
    results["ml_utility"] = ml_utility(
        ...
        test_fraction=test_fraction, seed=seed, model=ml_model,
    )

    if run_membership_inference:
        from src.evaluation.membership_inference import membership_inference_joined

        results["membership_inference"] = membership_inference_joined(
            real_data, synthetic_data, schema,
            seed=seed, join_clip_mult=join_clip_mult,
        )
